...
export const createAccount = async (req, res, next) => {
    try {
        ...
        if (existingAccounts.length > 0) {
            return res.status(STATUS_CODE.BAD_REQUEST).send("Sorry, one account per user.");
        }

        // Convert cash to a number and validate
        cash = Number(cash);
        if (isNaN(cash) || cash < 0) {
            return res.status(STATUS_CODE.BAD_REQUEST).send("Invalid cash value. Must be a non-negative number.");
        }
...
export const deleteAccount = async (req, res, next) => {
    try {
        ...
        if (accounts.length === 0) {
            return res.status(STATUS_CODE.NOT_FOUND).send("No accounts found for the user.");
        }

        // Delete all accounts associated with the user
        await Account.deleteMany({ owner: userId });

        res.status(STATUS_CODE.OK).send("All accounts deleted successfully for the user.");
    } catch (error) {
        next(error);
    }
}
...
export const userDeposit = async (req, res, next) => {
    try {
        ...
        if (isNaN(amount) || amount <= 0) {
            return res.status(STATUS_CODE.BAD_REQUEST).send("Invalid deposit amount. Must be a positive number.");
        }

        const checkingAccount = await Account.findOne({ owner: userId, accountType: 'checking' });
        if (!checkingAccount) {
            return res.status(STATUS_CODE.NOT_FOUND).send("Checking account not found.");
        }
...
export const userWithdraw = async (req, res, next) => {
    try {
        ...
        if (checkingAccount.cash < amount) {
            return res.status(STATUS_CODE.BAD_REQUEST).send("Insufficient funds for the withdrawal.");
        }
...
export const userTransfer = async (req, res, next) => {
    try {
        ...
        if (isNaN(transferAmount) || transferAmount <= 0) {
            return res.status(STATUS_CODE.BAD_REQUEST).send("Invalid transfer amount. Must be a positive number.");
        }

        // Find user's checking and savings accounts
        const checkingAccount = await Account.findOne({ owner: userId, accountType: 'checking' });
        const savingsAccount = await Account.findOne({ owner: userId, accountType: 'savings' });

        if (!checkingAccount || !savingsAccount) {
            return res.status(STATUS_CODE.NOT_FOUND).send("Checking or savings account not found.");
        }
        ...
        if (fromAccount.cash < transferAmount) {
            return res.status(STATUS_CODE.BAD_REQUEST).send(`Insufficient funds in the ${fromAccountId} account.`);
        }
